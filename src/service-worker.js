...
function openDb(){return new Promise((resolve,reject)=>{const request=indexedDB.open('pacefold-v26',DB_VERSION);request.onupgradeneeded=()=>{const db=request.result;if(!db.objectStoreNames.contains(STORE))db.createObjectStore(STORE)};request.onsuccess=()=>resolve(request.result);request.onerror=()=>reject(request.error)})}
...
async function networkFirst(request,fallback){const controller=new AbortController(),timer=setTimeout(()=>controller.abort(),4000);try{const response=await fetch(request,{signal:controller.signal});clearTimeout(timer);if(response?.ok)(await caches.open(CACHE_NAME)).put(request,response.clone()).catch(()=>{});return response}catch{clearTimeout(timer);return(await caches.match(request,{ignoreSearch:true}))||(await caches.match(path(fallback)))||Response.error()}}
self.addEventListener('fetch',event=>{
  ...
  if(url.pathname.endsWith('/app/daily-image.json')||url.pathname.endsWith('/app/daily-image.jpg')){event.respondWith(fetch(request,{cache:'no-store'}).catch(()=>caches.match(request,{ignoreSearch:true}).then(response=>response||Response.error())));return}
...
function backgroundCues(mirror,state,now=Date.now()){
  if(!mirror||!mirror.notifications||now<Number(state.snoozeUntil||0))return[];const cues=[];const add=cue=>{if(cue?.key&&!state.ack[cue.key]&&!state.notified[cue.key])cues.push(cue)};

...
const SHELL=[
  ...
  './app/','./app/index.html','./app/pacefold.css','./app/pacefold.mjs',
...
async function networkFirst(request,fallback){const controller=new AbortController(),timer=setTimeout(()=>controller.abort(),4000),fallbackUrl=path(fallback);try{const response=await fetch(request,{signal:controller.signal,cache:'no-store',credentials:'same-origin'});clearTimeout(timer);if(response?.ok)(await caches.open(CACHE_NAME)).put(fallbackUrl,response.clone()).catch(()=>{});return secureResponse(response)}catch{clearTimeout(timer);const cached=await caches.match(fallbackUrl);return cached?secureResponse(cached):Response.error()}}
self.addEventListener('fetch',event=>{
  ...
  const relative=url.pathname.startsWith(ROOT.pathname)?`/${url.pathname.slice(ROOT.pathname.length)}`:url.pathname;
  if(AUTH_PATHS.has(relative)){event.respondWith(fetch(request,{cache:'no-store',credentials:'same-origin'}).then(secureResponse));return}
  if(url.pathname.endsWith('/app/daily-image.json')||url.pathname.endsWith('/app/daily-image.jpg')){event.respondWith(fetch(request,{cache:'no-store',credentials:'same-origin'}).then(secureResponse).catch(()=>caches.match(request,{ignoreSearch:true}).then(response=>response?secureResponse(response):Response.error())));return}
  if(request.mode==='navigate'){event.respondWith(networkFirst(request,navigationFallback(url)));return}

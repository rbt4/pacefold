...
const CACHE_NAME=`pacefold-v${VERSION}-polish-r2`;
const ROOT=new URL('./',self.location.href);
const path=value=>new URL(value,ROOT).href;
const SHELL=[
  './','./index.html','./site.css','./privacy.html','./manifest.webmanifest',
  './app/','./app/index.html','./app/pacefold.css','./app/pacefold.mjs','./app/core.mjs','./app/auth.html','./app/auth.js',
  './app/fonts/pacefold-ma.woff2','./app/icons/fold-mark.svg','./app/icons/icon-192.png','./app/icons/icon-512.png',
  './app/icons/notify-water.svg','./app/icons/notify-prayer.svg','./app/icons/notify-prepare.svg','./app/icons/notify-away.svg','./app/icons/notify-meal.svg','./app/icons/notify-eyes.svg','./app/icons/notify-move.svg',
...
self.addEventListener('notificationclick',event=>{
  const data=event.notification.data||{};event.notification.close();event.waitUntil((async()=>{
    const windows=await self.clients.matchAll({type:'window',includeUncontrolled:true}),target=path('./app/?mode=now');
    if(event.action==='ack')for(const client of windows)client.postMessage({type:'PACEFOLD_ACK',source:data.source,key:data.key});
    for(const client of windows){if(client.url.startsWith(ROOT.href)){await client.focus();return}}if(self.clients.openWindow)await self.clients.openWindow(target);
  })());
});

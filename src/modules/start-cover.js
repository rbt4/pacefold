...
export function installStartCover(ctx){
  ...
  // The way in is a small live dial: the sun sits where it is on the 24-hour face.
  const NS='http://www.w3.org/2000/svg',mk=(tag,attrs)=>{const n=document.createElementNS(NS,tag);for(const[k,v]of Object.entries(attrs))n.setAttribute(k,String(v));return n};
  const mini=mk('svg',{viewBox:'-20 -20 40 40',class:'peel-dial','aria-hidden':'true'}),sun=mk('circle',{r:3.2,class:'peel-sun'});
  mini.append(mk('circle',{r:15,class:'peel-ring'}),mk('path',{d:'M-15 0H15',class:'peel-horizon'}),sun);
  const peel=button('cover-peel','Open Clock');peel.id='cover-peel';peel.append(mini,el('span','','Open Clock'));
  ...
  const updateInert=covered=>{const stage=id('stage'),edges=id('mobile-nav');if(stage)stage.inert=covered;if(edges)edges.inert=covered};
  ctx.setStartCover=(covered,{focus=false}={})=>{document.documentElement.dataset.cover=covered?'on':'peeled';cover.setAttribute('aria-hidden',String(!covered));restore.setAttribute('aria-hidden',String(covered));updateInert(covered);if(covered&&focus)requestAnimationFrame(()=>searchInput.focus({preventScroll:true}))};
  const tick=()=>{const now=new Date(),parts=new Intl.DateTimeFormat(undefined,{timeZone:ctx.prefs.timeZone,hour:'numeric',minute:'2-digit',second:'2-digit',hour12:ctx.prefs.timeFormat!=='24'}).formatToParts(now),read=type=>parts.find(part=>part.type===type)?.value||'',hour=Number(new Intl.DateTimeFormat('en-CA',{timeZone:ctx.prefs.timeZone,hour:'2-digit',hourCycle:'h23'}).format(now));mainTime.textContent=`${read('hour')}:${read('minute')}`;second.textContent=read('second');period.textContent=read('dayPeriod');period.hidden=!read('dayPeriod');time.dateTime=now.toISOString();date.textContent=new Intl.DateTimeFormat(undefined,{timeZone:ctx.prefs.timeZone,weekday:'long',month:'long',day:'numeric'}).format(now);if(now.getSeconds()===0||!sun.hasAttribute('cx')){const sun24=ctx.sunHours?.(now,ctx.prefs),noonAt=sun24?.noon??12,frac=hour+now.getMinutes()/60,a=-Math.PI/2+((frac-noonAt)/24)*2*Math.PI;sun.setAttribute('cx',(Math.cos(a)*15).toFixed(2));sun.setAttribute('cy',(Math.sin(a)*15).toFixed(2))}cover.dataset.phase=hour<6?'night':hour<11?'morning':hour<17?'day':hour<21?'evening':'night'};tick();setInterval(tick,1000);
  ...
  peel.addEventListener('click',()=>ctx.setStartCover(false));restore.addEventListener('click',()=>ctx.setStartCover(true,{focus:false}));let startY=0;peel.addEventListener('pointerdown',event=>{startY=event.clientY;peel.setPointerCapture?.(event.pointerId)});peel.addEventListener('pointerup',event=>{if(startY&&event.clientY-startY<-24)ctx.setStartCover(false);startY=0});
  // Scrolling down, or swiping up, on the cover also opens Clock.
  let wheel=0;cover.addEventListener('wheel',event=>{if(document.documentElement.dataset.cover!=='on'||event.target.closest?.('.cover-note-panel'))return;wheel=Math.max(0,wheel+event.deltaY);if(wheel>120){wheel=0;ctx.setStartCover(false)}},{passive:true});
  let touchY=0;cover.addEventListener('touchstart',event=>{touchY=event.touches[0]?.clientY||0},{passive:true});cover.addEventListener('touchend',event=>{const y=event.changedTouches[0]?.clientY||0;if(touchY&&touchY-y>70&&!event.target.closest?.('input,textarea,.cover-week'))ctx.setStartCover(false);touchY=0},{passive:true});

...
export function installStartCover(ctx){
  ...
  const utility=el('section','cover-utility');utility.setAttribute('aria-label','Search and quick note');
  ...
  const noteActions=el('div','cover-note-actions');const noteCancel=button('','Close quick note','Cancel');const noteKeep=button('primary','Keep note','Keep');noteKeep.type='submit';noteActions.append(noteCancel,noteKeep);noteForm.append(noteInput,noteActions);
  utility.append(searchForm,noteForm);

  const peel=button('cover-peel','Open Pacefold');peel.id='cover-peel';peel.append(el('i'),el('span','','Open Pacefold'),el('b','','⌃'));
  const credit=el('a','cover-photo-credit','');credit.id='cover-photo-credit';credit.target='_blank';credit.rel='noopener noreferrer';credit.hidden=true;
  hero.append(clock,utility);cover.append(backdrop,hero,credit,peel);document.body.append(cover);
  ...
  const tick=()=>{
    const now=new Date(),parts=new Intl.DateTimeFormat(undefined,{timeZone:ctx.prefs.timeZone,hour:'numeric',minute:'2-digit',second:'2-digit',hour12:ctx.prefs.timeFormat!=='24'}).formatToParts(now),read=type=>parts.find(part=>part.type===type)?.value||'';
    mainTime.textContent=`${read('hour')}:${read('minute')}`;second.textContent=read('second');period.textContent=read('dayPeriod');period.hidden=!read('dayPeriod');time.dateTime=now.toISOString();
  ...
  const loadDailyImage=async()=>{
    try{
      const response=await fetch('./daily-image.json',{cache:'no-store'});if(!response.ok)return;const data=await response.json();
      if(!/^https:\/\/www\.bing\.com\//i.test(data.url||''))return;
      cover.style.setProperty('--cover-image',`url("${String(data.url).replaceAll('"','%22')}")`);cover.dataset.photo='loaded';
      if(data.credit){credit.textContent='ⓘ  '+data.credit;credit.title=data.credit;credit.href=/^https:\/\//i.test(data.creditUrl||'')?data.creditUrl:'https://www.bing.com/';credit.hidden=false}
    }catch{}
  };void loadDailyImage();
  ...
  const resolveTarget=value=>{
    ...
    if(/^(localhost|([\w-]+\.)+[a-z]{2,})(:\d+)?(\/|$)/i.test(input))return`https://${input}`;
    return`${GOOGLE_SEARCH}${encodeURIComponent(input)}`;
  };
  searchForm.addEventListener('submit',event=>{event.preventDefault();const href=resolveTarget(searchInput.value);if(!href)return;const link=document.createElement('a');link.href=href;link.target='_blank';link.rel='noopener noreferrer';document.body.append(link);link.click();link.remove();searchInput.select()});
  ...
  const sound=id('sound-bar');if(sound&&!id('music-launch')){sound.dataset.streaming='true';sound.setAttribute('aria-label','Music');const launch=el('a','music-launch');launch.id='music-launch';launch.href=YOUTUBE_MUSIC;launch.target='_blank';launch.rel='noopener noreferrer';launch.setAttribute('aria-label','Open YouTube Music in a new tab');const mark=el('i','music-mark');mark.setAttribute('aria-hidden','true');const copy=el('span');copy.append(el('small','','Music'),el('strong','','YouTube Music'));launch.append(mark,copy,el('b','','↗'));sound.append(launch);for(const child of[...sound.children])if(child!==launch)child.hidden=true}

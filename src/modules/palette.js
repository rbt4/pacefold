...
export function installPalette(ctx){
  ...
  const commands=()=>[
    ...
    {group:'Look',label:'Preview rain',tone:'water',run:()=>{ctx.setStartCover?.(false);ctx.previewAtmosphere?.('rain')},words:'weather effect demo'},
    {group:'Look',label:'Preview a thunderstorm',tone:'water',run:()=>{ctx.setStartCover?.(false);ctx.previewAtmosphere?.('storm')},words:'lightning weather effect demo'},
    {group:'Look',label:'Let it snow',tone:'water',run:()=>{ctx.setStartCover?.(false);ctx.previewAtmosphere?.('snow')},words:'snow weather effect demo'},
    {group:'Look',label:'Preview fog',tone:'water',run:()=>{ctx.setStartCover?.(false);ctx.previewAtmosphere?.('fog')},words:'mist weather effect demo'},
    {group:'Look',label:'Preview the night sky',tone:'sun',run:()=>{ctx.setStartCover?.(false);ctx.previewAtmosphere?.('night')},words:'stars shooting star demo'},

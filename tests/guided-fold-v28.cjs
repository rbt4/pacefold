...
for(const token of["EXPERIENCE='v28-guided-fold-r1'","HOVER_DWELL=640","ctx.go?.(destination,{directional:true})","v28-cue-peek","v28-cue-bloom","Care defaults","Recovery & diagnostics","Calculation & location details"])assert(source.includes(token),`Guided Fold source contract missing: ${token}`);
...
assert(style.includes('.v28-guide')&&style.includes('.v28-cue-peek')&&style.includes('.now-quick')&&style.includes('.v28-hover-commit')&&style.includes('.v28-settings-disclosure'),'Guided Fold surfaces are not styled by the single stylesheet');

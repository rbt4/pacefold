...
async function main(){
  ...
  try{
    ...
    const home=await page.evaluate(()=>({
      ...
      svg:Boolean(document.getElementById('day-arc-path')&&document.getElementById('day-sun-group')),polish:['note-insights','day-story-title','now-cue-list','settings-summary','clock-note-input'].every(id=>document.getElementById(id))
    }));
    ...
    assert(home.release==='26.0.0'&&home.notes===1,'Migration did not preserve the V26 release or note');assert(home.discretion==='neutral','Existing installs must default to neutral Clock discretion');assert(home.rhythmMeta==='','Clock rhythm metadata is not empty');
    assert(!/\b(Fajr|Dhuhr|Asr|Maghrib|Isha|Hanafi|prayer)\b|Etobicoke|Toronto|America\/Toronto|15°/i.test(home.clockText),'Neutral Clock leaked prayer, method or location vocabulary');assert(home.svg&&home.polish,'V26 Clock surfaces are incomplete');
    ...
    await page.fill('#note-input','Pacefold 26 browser audit note');await page.click('#note-form button[type="submit"]');await page.waitForTimeout(100);
    const savedId=await page.evaluate(()=>window.__PACEFOLD__.notes.find(note=>note.body==='Pacefold 26 browser audit note')?.id);assert(savedId,'Note was not saved');
    ...
    await editor.fill('Pacefold 26 inline edit verified');assert((await page.locator('#note-save-status').textContent()).includes('Unsaved'),'Inline edit did not expose dirty state');
    await page.locator(`[data-note-id="${savedId}"] button[aria-label="Save note changes"]`).click();assert(await page.evaluate(()=>window.__PACEFOLD__.notes.some(note=>note.body==='Pacefold 26 inline edit verified')),'Inline note edit did not save');
    ...
    await page.click('[data-setting="showSeconds"]');await page.reload({waitUntil:'networkidle'});await page.waitForFunction(()=>window.__PACEFOLD__?.version==='26.0.0');assert(await page.evaluate(()=>window.__PACEFOLD__.prefs.showSeconds===false),'Seconds preference did not persist');

    await page.evaluate(()=>window.__PACEFOLD__.go('home'));await page.screenshot({path:path.join(artifacts,'pacefold-26-home.png'),fullPage:true});
    for(const view of['notes','worklog','now','settings']){await page.evaluate(next=>window.__PACEFOLD__.go(next),view);await page.waitForTimeout(80);const overflow=await page.evaluate(()=>document.documentElement.scrollWidth-innerWidth);assert(overflow<=1,`${view} overflows by ${overflow}px`);await page.screenshot({path:path.join(artifacts,`pacefold-26-${view}.png`),fullPage:true})}
    ...
    await m.screenshot({path:path.join(artifacts,'pacefold-26-mobile.png'),fullPage:true});for(const view of['notes','worklog','now','settings']){await m.evaluate(next=>window.__PACEFOLD__.go(next),view);await m.waitForTimeout(60);const overflow=await m.evaluate(()=>document.documentElement.scrollWidth-innerWidth);assert(overflow<=1,`Mobile ${view} overflows by ${overflow}px`);await m.screenshot({path:path.join(artifacts,`pacefold-26-mobile-${view}.png`),fullPage:true})}
    await mobile.close();console.log('Pacefold 26 browser audit passed: modular runtime, discreet Clock, current Daybook, persistence and touch navigation.');

...
async function main(){
  ...
  try{
    ...
    assert(!/\b(Fajr|Dhuhr|Asr|Maghrib|Isha|Hanafi|prayer)\b|Etobicoke|Toronto|America\/Toronto|15°/i.test(home.clockText),'Neutral Clock leaked prayer, method or location vocabulary');assert(home.svg&&home.polish,'V27 Clock surfaces are incomplete');assert(home.brand==='Clock'&&home.title.startsWith('Clock'),'Ambient chrome is not discreet');
    ...
    const nowText=await page.locator('[data-view="now"]').innerText();assert(!/\b(Fajr|Dhuhr|Asr|Maghrib|Isha|Hanafi|prayer)\b/i.test(nowText),'Neutral Now view leaked rhythm names');
    ...
    await page.evaluate(()=>window.__PACEFOLD__.go('home'));await page.screenshot({path:path.join(artifacts,'pacefold-27-home.png'),fullPage:true});
    for(const view of['notes','worklog','now','settings']){await page.evaluate(next=>window.__PACEFOLD__.go(next),view);await page.waitForTimeout(80);const overflow=await page.evaluate(()=>document.documentElement.scrollWidth-innerWidth);assert(overflow<=1,`${view} overflows by ${overflow}px`);if(view==='worklog')assert(await page.locator('#day-compare').count()===1,'Day comparison surface missing');await page.screenshot({path:path.join(artifacts,`pacefold-27-${view}.png`),fullPage:true})}
    ...
    await m.screenshot({path:path.join(artifacts,'pacefold-27-mobile.png'),fullPage:true});for(const view of['notes','worklog','now','settings']){await m.evaluate(next=>window.__PACEFOLD__.go(next),view);await m.waitForTimeout(60);const overflow=await m.evaluate(()=>document.documentElement.scrollWidth-innerWidth);assert(overflow<=1,`Mobile ${view} overflows by ${overflow}px`);await m.screenshot({path:path.join(artifacts,`pacefold-27-mobile-${view}.png`),fullPage:true})}

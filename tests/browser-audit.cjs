...
async function main(){
  ...
  try{
    ...
    const home=await page.evaluate(()=>({
      mode:document.documentElement.dataset.mode,cover:document.documentElement.dataset.cover,
    ...
    await page.click('#cover-peel');await page.waitForFunction(()=>document.documentElement.dataset.cover==='peeled');assert(await page.evaluate(()=>document.getElementById('stage').inert===false),'Peeling cover did not reactivate Pacefold');

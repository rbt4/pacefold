...
assert(release.includes("RELEASE='29.0.0'")&&release.includes("REVISION='unified-recovery-r1'"),'Canonical release metadata is wrong');
assert(app.includes('pacefold.css?v=29.0.0')&&app.includes('pacefold.mjs?v=29.0.0'),'App cache-busting identity is stale');
assert(worker.includes("VERSION='29.0.0'")&&worker.includes('unified-recovery-r1'),'Worker cache identity is stale');
...
console.log(JSON.stringify({release:'29.0.0',revision:'unified-recovery-r1',cssBytes:css.length,startup:'returning users open on Clock',visualCascade:'explicit manifest'},null,2));

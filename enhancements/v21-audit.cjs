...
async function browserAudit(){
  ...
  try{
    ...
    await page.waitForFunction(release=>window.__PACEFOLD_V21__?.release===release&&window.__PACEFOLD_V21_PERSISTENCE__?.release===release&&window.__PACEFOLD_V21_PRECISION__?.experience===release&&document.querySelectorAll('.pf21-calendar-day').length===42&&document.getElementById('pf21-settings'),RELEASE);
    ...
    await page.reload({waitUntil:'networkidle'});await page.waitForFunction(release=>window.__PACEFOLD_V21_PRECISION__?.experience===release&&document.getElementById('pf21-note-calendar'),RELEASE);

...
function staticAudit(){
  ...
  for(const token of ['pf22-spatial-root','pf22-face-notes','pf22-face-worklog','pf22-face-context','pf22-face-settings','pacefold:spatial-ready'])assert(js.includes(token),`Spatial runtime token missing: ${token}`);
  ...
  assert(css.includes('Clock-first')&&css.includes('.pf22-stage'),'Spatial CSS identity missing');
}

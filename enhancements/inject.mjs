...
async function applyRuntimePatch(file){
  ...
  const replacements=[
    ...
    ["function toggleNotebook(){notebookState.open=!(notebookState.open!==false);saveNotebookState();workspace?.classList.toggle('is-open',notebookState.open);workspace?.setAttribute('data-open',String(notebookState.open));if(notebookState.open)setTimeout(()=>workspace?.querySelector('[data-pf-note-body]')?.focus({preventScroll:true}),80);}","function setNotebookOpen(open,focus=true){const next=Boolean(open);clearTimeout(notebookMotionTimer);notebookState.open=next;saveNotebookState();if(!workspace)return;workspace.dataset.foldMotion=next?'opening':'closing';workspace.classList.toggle('is-open',next);workspace.setAttribute('data-open',String(next));workspace.querySelector('[data-pf-revamp-title]')?.setAttribute('aria-expanded',String(next));notebookMotionTimer=setTimeout(()=>{notebookMotionTimer=0;if(workspace?.dataset.foldMotion===(next?'opening':'closing'))delete workspace.dataset.foldMotion;},620);if(next&&focus)setTimeout(()=>{if(notebookState.open)workspace?.querySelector('[data-pf-note-body]')?.focus({preventScroll:true});},360);}\nfunction toggleNotebook(){setNotebookOpen(!(notebookState.open!==false));}",'notebook-motion'],
    ...
    ["function setPlayerDrawer(open){playerState.drawer=open;savePlayerState();const drawer=player?.querySelector('[data-pf-player-drawer]');if(drawer)drawer.hidden=!open;player?.classList.toggle('is-open',open);if(open)renderPlayerDrawer();}","function setPlayerDrawer(open){const next=Boolean(open);clearTimeout(playerMotionTimer);playerState.drawer=next;savePlayerState();if(player){player.dataset.foldMotion=next?'opening':'closing';player.classList.toggle('is-open',next);player.querySelector('[data-pf-player-menu]')?.setAttribute('aria-expanded',String(next));}const drawer=player?.querySelector('[data-pf-player-drawer]');if(drawer)drawer.hidden=!next;playerMotionTimer=setTimeout(()=>{playerMotionTimer=0;if(player?.dataset.foldMotion===(next?'opening':'closing'))delete player.dataset.foldMotion;},620);if(next)renderPlayerDrawer();}",'player-motion'],
...
try{
  ...
  console.log(`Installed Pacefold ${RELEASE}: kinetic origami workspace with staged notebook and audio folds.`);
}finally{

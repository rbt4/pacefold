...
const maJsSource=path.join(sourceRoot,'pacefold-ma.js');
const themeBootSource=path.join(sourceRoot,'pf-theme-boot.js');
const layoutPatch=`
...
async function replaceMarkedBlock(file,marker,content,legacyMarkers=[]){
  ...
  for(const candidate of [marker,...legacyMarkers]){
    const start=`/* BEGIN ${candidate} */`,end=`/* END ${candidate} */`;
    let startIndex=source.indexOf(start);
    while(startIndex>=0){const endIndex=source.indexOf(end,startIndex);if(endIndex<0)throw new Error(`Pacefold ${candidate} end marker is missing`);source=source.slice(0,startIndex)+source.slice(endIndex+end.length);startIndex=source.indexOf(start);}
  }
  source=source.replace(/\s+$/,'')+`\n\n/* BEGIN ${marker} */\n${content.trim()}\n/* END ${marker} */\n`;
  await fs.writeFile(file,source);
}
async function applyLayoutPatch(file){const inner=layoutPatch.trim().replace(`/* BEGIN ${layoutMarker} */`,'').replace(`/* END ${layoutMarker} */`,'').trim();await replaceMarkedBlock(file,layoutMarker,inner);}
async function applyOrigamiPatch(file){const identity=await fs.readFile(origamiSource,'utf8'),polish=await fs.readFile(origamiPolishSource,'utf8');await replaceMarkedBlock(file,origamiMarker,`${identity.trim()}\n\n${polish.trim()}`,legacyOrigamiMarkers);}
async function applyStabilityPatch(file){await replaceMarkedBlock(file,stabilityMarker,await fs.readFile(stabilitySource,'utf8'),legacyStabilityMarkers);}
async function applyAssetRevision(file){
  let html=await fs.readFile(file,'utf8');
  html=html.replace(/((?:\.\/)?pacefold-(?:hub(?:-guardian)?|resilience|integrated|revamp)\.(?:css|js))\?v=[^"'&<\s]+/g,(_,asset)=>`${asset}?v=${RELEASE}`);
  html=html.replace(/\s*<meta\s+name=["']pacefold-build["'][^>]*>/gi,'');
  html=html.replace('</head>',`  <meta name="pacefold-build" content="${RELEASE}">\n</head>`);
  await fs.writeFile(file,html);
}
async function injectMaAssets(file){
  let html=await fs.readFile(file,'utf8');
  html=html.replace(/\s*<link[^>]+href=["'][^"']*pacefold-ma\.css[^"']*["'][^>]*>/gi,'');
  html=html.replace(/\s*<script[^>]+src=["'][^"']*(?:pf-theme-boot|pacefold-ma)\.js[^"']*["'][^>]*><\/script>/gi,'');
  const boot=`  <script src="./pf-theme-boot.js?v=${RELEASE}"></script>\n  <link rel="stylesheet" href="./pacefold-ma.css?v=${RELEASE}">\n`;
  html=html.replace(/<head([^>]*)>/i,match=>`${match}\n${boot}`);
  const runtime=`  <script src="./pacefold-ma.js?v=${RELEASE}" defer></script>\n`;
  const appScript=/<script\b[^>]*\bsrc=["'](?:\.\/)?app\.js(?:\?[^"']*)?["'][^>]*><\/script>/i;
  if(!appScript.test(html))throw new Error('Pacefold core app script tag is missing');
  html=html.replace(appScript,match=>`${runtime}${match}`);
  await fs.writeFile(file,html);
}
async function patchManifests(root){
  if(!(await exists(root)))return;
  for(const name of await fs.readdir(root)){
    if(!/^manifest.*(?:json|webmanifest)$/i.test(name))continue;
    const file=path.join(root,name);let manifest;
...
async function stampWorker(file){
  ...
  worker+=`\n\n// BEGIN ${workerReleaseMarker}\nself.__PACEFOLD_SURFACE_RELEASE__='${RELEASE}';\nconst __PACEFOLD_MA_ASSETS__=(self.location.pathname.includes('/app/')?['./pacefold-ma.css','./pacefold-ma.js','./pf-theme-boot.js']:['./app/pacefold-ma.css','./app/pacefold-ma.js','./app/pf-theme-boot.js']).map(value=>new URL(value,self.location.href).href);\nself.addEventListener('install',event=>event.waitUntil(caches.open('pacefold-ma-${RELEASE}').then(cache=>cache.addAll(__PACEFOLD_MA_ASSETS__)).catch(()=>{})));\n// END ${workerReleaseMarker}\n`;
  await fs.writeFile(file,worker);
}
function replaceExactlyOnce(source,from,to,label){const first=source.indexOf(from);if(first<0||source.indexOf(from,first+from.length)>=0)throw new Error(`Pacefold runtime patch ${label} expected exactly one source match`);return source.slice(0,first)+to+source.slice(first+from.length);}
async function applyRuntimePatch(file){
  let runtime=await fs.readFile(file,'utf8');
  const replacements=[
    ["const STREAM_KEY='pacefold.player.streaming-links.v1';\nconst WORK_OVERRIDE_KEY", "const STREAM_KEY='pacefold.player.streaming-links.v1';\nconst NOTEBOOK_DRAFT_KEY='pacefold.notebook.draft.v1';\nconst VISUAL_RESET_KEY='pacefold.visual-reset.18.0.0';\nconst WORK_OVERRIDE_KEY",'visual-reset-key'],
...
try{
  ...
  await fs.copyFile(maCssSource,path.join(targetApp,'pacefold-ma.css'));
  await fs.copyFile(maJsSource,path.join(targetApp,'pacefold-ma.js'));
  await fs.copyFile(themeBootSource,path.join(targetApp,'pf-theme-boot.js'));
  await applyRuntimePatch(path.join(targetApp,'pacefold-revamp.js'));
  await injectMaAssets(path.join(targetApp,'index.html'));
  await applyAssetRevision(path.join(targetApp,'index.html'));
  await patchManifests(targetRoot);
  await patchManifests(targetApp);
  ...
  console.log(`Installed Pacefold ${RELEASE}: Ma day ribbon, one cue scheduler, wafer mode and local review.`);

...
source=source.replace(capturedVisible,"assert(!await page.locator('#pf-local-workspace').evaluate(node=>node.classList.contains('is-open')),'Quiet capture forced the full notebook open');await page.evaluate(()=>window.__PACEFOLD_REVAMP__.openNotebook());await page.waitForSelector('#pf-local-workspace.is-open');assert(await page.getByText('Flow audit note').isVisible(),'Captured note is not visible after opening the integrated notebook');const draftComposer=page.locator('[data-pf-note-body]');await draftComposer.fill('Draft survives folding');await page.keyboard.press('Escape');await page.waitForFunction(()=>!document.getElementById('pf-local-workspace').classList.contains('is-open'));assert((await page.evaluate(()=>JSON.parse(localStorage.getItem('pacefold.notebook.draft.v1')||'{}').body))==='Draft survives folding','Unsaved notebook draft was not retained');await page.evaluate(()=>window.__PACEFOLD_REVAMP__.openNotebook());await page.waitForSelector('#pf-local-workspace.is-open');assert(await draftComposer.inputValue()==='Draft survives folding','Folded notebook draft was not restored');await draftComposer.fill('');");
...
source=source.replace(researchVisible,`${researchVisible}await page.waitForFunction(()=>!document.getElementById('pf-local-workspace').classList.contains('is-open'),{timeout:3500});const foldedState=await page.evaluate(()=>JSON.parse(localStorage.getItem('pacefold.notebook.ui.v1')||'{}'));assert(foldedState.open===false,'Notebook persisted an open state after auto-close');await page.evaluate(()=>window.__PACEFOLD_REVAMP__.openNotebook());await page.waitForSelector('#pf-local-workspace.is-open');`);
...
source=source.replace(playerOpen,`await page.evaluate(()=>window.__PACEFOLD_REVAMP__.player.open());await page.waitForSelector('[data-pf-player-drawer]:visible');await page.waitForTimeout(300);const exclusive=await page.evaluate(()=>{const workspace=document.getElementById('pf-local-workspace'),drawer=document.querySelector('[data-pf-player-drawer]'),player=document.getElementById('pf-local-player'),wr=workspace.getBoundingClientRect(),dr=drawer.getBoundingClientRect(),pr=player.getBoundingClientRect(),menu=document.querySelector('[data-pf-player-menu]'),v19=Boolean(window.__PACEFOLD_V19__);return {v19,notebookOpen:workspace.classList.contains('is-open'),workspaceAboveDrawer:wr.bottom<=dr.top+2,widthAligned:Math.abs(wr.left-dr.left)<=2&&Math.abs(wr.right-dr.right)<=2,playerInView:pr.left>=0&&pr.top>=0&&pr.right<=innerWidth&&pr.bottom<=innerHeight,expanded:menu.getAttribute('aria-expanded'),label:menu.getAttribute('aria-label')};});assert(!exclusive.notebookOpen&&(exclusive.v19?exclusive.playerInView:exclusive.workspaceAboveDrawer&&exclusive.widthAligned)&&exclusive.expanded==='true'&&/^Close/.test(exclusive.label),\`Notebook/music exclusivity failed: \${JSON.stringify(exclusive)}\`);`);
...
source=source.replace(desktopCapture,`${desktopCapture}
    await page.evaluate(()=>window.__PACEFOLD_REVAMP__.closeNotebook());await page.waitForTimeout(300);
    ...
    await page.evaluate(()=>window.__PACEFOLD_REVAMP__.openNotebook());await page.waitForTimeout(90);
    const foldVisual=await page.evaluate(()=>{const workspace=document.getElementById('pf-local-workspace'),sheet=workspace.querySelector('.pf-notebook-sheet'),style=getComputedStyle(window.__PACEFOLD_V19__?workspace:sheet),rect=workspace.getBoundingClientRect();return {v19:Boolean(window.__PACEFOLD_V19__),motion:workspace.dataset.foldMotion,transform:style.transform,opacity:Number(style.opacity),open:workspace.classList.contains('is-open'),inView:rect.left>=0&&rect.top>=0&&rect.right<=innerWidth&&rect.bottom<=innerHeight};});
    assert(foldVisual.open&&(foldVisual.v19?foldVisual.inView&&foldVisual.opacity>0:foldVisual.motion==='opening'&&foldVisual.transform!=='none'&&foldVisual.opacity>0&&foldVisual.opacity<1),'Notebook opening did not render as a contained live transition: '+JSON.stringify(foldVisual));
    await page.screenshot({path:path.join(artifactRoot,'pacefold-workspace-fold-midpoint.png'),fullPage:true});
    await page.waitForTimeout(220);
    await page.evaluate(()=>window.__PACEFOLD_REVAMP__.player.open());await page.waitForSelector('[data-pf-player-drawer]:visible');await page.waitForTimeout(300);
    const musicVisual=await page.evaluate(()=>{const workspace=document.getElementById('pf-local-workspace'),drawer=document.querySelector('[data-pf-player-drawer]'),player=document.getElementById('pf-local-player'),wr=workspace.getBoundingClientRect(),dr=drawer.getBoundingClientRect(),pr=player.getBoundingClientRect(),v19=Boolean(window.__PACEFOLD_V19__);return {v19,notebookOpen:workspace.classList.contains('is-open'),coverAboveDrawer:wr.bottom<=dr.top+2,drawerAbovePlayer:dr.bottom<=pr.top+2,widthAligned:Math.abs(wr.left-dr.left)<=2&&Math.abs(wr.right-dr.right)<=2&&Math.abs(dr.left-pr.left)<=2&&Math.abs(dr.right-pr.right)<=2,inView:pr.left>=0&&pr.top>=0&&pr.right<=innerWidth&&pr.bottom<=innerHeight,background:getComputedStyle(player).backgroundColor};});
    assert(!musicVisual.notebookOpen&&(musicVisual.v19?musicVisual.inView&&!/rgb\\(0,\\s*0,\\s*0\\)/.test(musicVisual.background):musicVisual.coverAboveDrawer&&musicVisual.drawerAbovePlayer&&musicVisual.widthAligned),\`Music visual geometry failed: \${JSON.stringify(musicVisual)}\`);
...
source=source.replace(mobileCapture,`${mobileCapture}
    await mobile.evaluate(()=>window.__PACEFOLD_REVAMP__.player.open());await mobile.waitForSelector('[data-pf-player-drawer]:visible');await mobile.waitForTimeout(300);
    const mobileMusic=await mobile.evaluate(()=>{const workspace=document.getElementById('pf-local-workspace'),drawer=document.querySelector('[data-pf-player-drawer]'),player=document.getElementById('pf-local-player'),wr=workspace.getBoundingClientRect(),dr=drawer.getBoundingClientRect(),pr=player.getBoundingClientRect(),v19=Boolean(window.__PACEFOLD_V19__);return {v19,notebookOpen:workspace.classList.contains('is-open'),coverAboveDrawer:wr.bottom<=dr.top+2,drawerAbovePlayer:dr.bottom<=pr.top+2,widthAligned:Math.abs(wr.left-dr.left)<=2&&Math.abs(wr.right-dr.right)<=2&&Math.abs(dr.left-pr.left)<=2&&Math.abs(dr.right-pr.right)<=2,inView:pr.left>=0&&pr.top>=0&&pr.right<=innerWidth&&pr.bottom<=innerHeight,overflow:document.documentElement.scrollWidth>innerWidth+2};});
    assert(!mobileMusic.notebookOpen&&(mobileMusic.v19?mobileMusic.inView:mobileMusic.coverAboveDrawer&&mobileMusic.drawerAbovePlayer&&mobileMusic.widthAligned)&&!mobileMusic.overflow,\`Mobile music visual geometry failed: \${JSON.stringify(mobileMusic)}\`);
...
const productRhythmGate=[
  ...
  "    await product.waitForFunction(()=>window.__PACEFOLD_REVAMP__?.surfaceRelease==='19.0.0'&&window.__PACEFOLD_V19__?.release==='19.0.0');",
  ...
  "    assert(!rhythmHome.notebookOpen&&!rhythmHome.playerOpen&&rhythmHome.visible.length===6&&rhythmHome.opacity>=.5&&!rhythmHome.overflow&&/30m|30-minute/i.test(`${rhythmHome.noodleText} ${rhythmHome.labels[1]}`),`Original rhythm home failed: ${JSON.stringify(rhythmHome)}`);",

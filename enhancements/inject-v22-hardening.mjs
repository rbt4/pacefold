...
async function patchWorker(file,{root=false}={}){
  ...
  const prefix=root?'./app/':'./',spatial=`'${prefix}pacefold-v22-spatial.js'`,assets=[`'${prefix}${cssName}'`,`'${prefix}${jsName}'`];
...
async function verify(){
  ...
  for(const asset of [cssName,jsName])if(!worker.includes(asset))throw new Error(`Offline shell omits ${asset}`);
  if(!worker.includes(`revision:${REVISION}`))throw new Error('Offline shell hardening revision is stale');
  for(const token of [`const RELEASE='${RELEASE}'`,'setNotifications','openSound','pf22-sound-overlay'])if(!js.includes(token))throw new Error(`Hardening runtime token missing: ${token}`);
}
...
await verify();
console.log(`Installed Pacefold ${RELEASE} Spatial Fold control hardening.`);

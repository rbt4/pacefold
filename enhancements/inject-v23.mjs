...
async function verify(){
  ...
  for(const token of [`const RELEASE='${RELEASE}'`,'complete-stabilization-r4','__PACEFOLD_ACTIVE_RELEASE__','observeReleaseTruth','window.__PACEFOLD_ACTIVE_RELEASE__||RELEASE','window.__PACEFOLD_ACTIVE_RELEASE__||EXPERIENCE'])if(!runtime.includes(token))throw new Error(`Stability runtime token missing: ${token}`);
}

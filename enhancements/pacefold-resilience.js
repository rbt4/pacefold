...
function installOneNoteGuard(){
  ...
  adapter.syncPage=function guardedSyncPage(payload){
    ...
    const timeout=new Promise((_,reject)=>setTimeout(()=>reject(new Error('Pacefold OneNote sync timed out after 25 seconds.')),25000));
    const task=Promise.race([Promise.resolve().then(()=>original(payload)),timeout])
      ...
      .finally(()=>{inFlightSyncs.delete(fingerprint);clearAction('sync-page');});
...
function pollForOneNoteAdapter(){
  if(installOneNoteGuard()){clearInterval(adapterPoll);adapterPoll=0;return;}

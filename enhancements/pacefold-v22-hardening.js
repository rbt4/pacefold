...
function toggleControl(key){
  try{
    const current=prefs();
    if(key==='quiet'){window.__PACEFOLD_MA_QUIET__?.toggle?.();setTimeout(()=>{sync();if(window.__PACEFOLD_MA_QUIET__?.get?.())window.__PACEFOLD_SPATIAL__?.home?.()},0);return}
    if(key==='weather'){const state=window.__PACEFOLD_V21_PERSISTENCE__?.read?.()||{},next=state.v21WeatherEnabled===false;window.__PACEFOLD_V21_PERSISTENCE__?.write?.({...state,v21WeatherEnabled:next});writePrefs({v21WeatherEnabled:next},'weather');showStatus(next?'Weather restored':'Weather hidden');return}

...
function splitStatementPrefix(lhs){const match=lhs.match(/^((?:(?:else\s+)?if\s*\([^;\n]*\)\s+|else\s+))(.+)$/s);return match?{prefix:match[1],target:match[2].trim()}:{prefix:'',target:lhs};}
export function rewriteInnerHTMLAssignments(input,setter='window.__PACEFOLD_SET_HTML__'){
  ...
  while(true){
    ...
    const dot=found.index,lhsStart=findLhsStart(source,dot),lhs=source.slice(lhsStart,dot).trim(),statement=splitStatementPrefix(lhs),target=statement.target;
    if(!target||/\b(?:if|for|while|return)\b/.test(target))fail(`Unsafe innerHTML left-hand expression: ${lhs}`);
    const rhsStart=match.lastIndex,end=findExpressionEnd(source,rhsStart),rhs=source.slice(rhsStart,end).trim(),leading=source.slice(lhsStart,dot).match(/^\s*/)?.[0]||'',replacement=`${leading}${statement.prefix}${setter}(${target},${rhs})`;

...
for(const name of ['pacefold-hub.js','pacefold-hub-guardian.js','pacefold-resilience.js','pacefold-integrated.js','pacefold-theme-boot.js','pacefold-ma.js','pacefold-v19.js','pacefold-v20.js','pacefold-v21-boot.js','pacefold-v22-spatial.js','pacefold-v22-hardening.js','pacefold-v22-cues.js','pacefold-v22-daylight.js','pacefold-v23-bootstrap.js','pacefold-v23-stability.js','pacefold-v23-action-dock.js','pacefold-v23-experience.js','pacefold-v23.js']){
...
if((injectedHtml.match(/data-pacefold-v23-bootstrap="23\.0\.0"/g)||[]).length!==1||(injectedHtml.match(/data-pacefold-v23-boot="23\.0\.0"/g)||[]).length!==1||(injectedHtml.match(/data-pacefold-v23-css="23\.0\.0"/g)||[]).length!==1||(injectedHtml.match(/data-pacefold-v23-runtime="23\.0\.0"/g)||[]).length!==1)fail('V23 active assets were not injected exactly once');
if((injectedHtml.match(/<link[^>]+rel="stylesheet"/g)||[]).length!==13||(injectedHtml.match(/<script[^>]+src=/g)||[]).length!==14)fail('V23 active request consolidation regressed');
if(injectedHtml.indexOf('pacefold-v23-bootstrap.js')>injectedHtml.indexOf('<script src="./app.js" defer></script>'))fail('V23 bootstrap does not precede app.js');
...
if(!landingHtml.includes('Pacefold 23 · one quiet spatial workday')||!landingHtml.includes('The clock always stays at the centre.')||!landingHtml.includes('automatic JSON backup')||!landingHtml.includes('Where do my notes go?'))fail('V23 landing page does not describe the unified spatial product');
...
if(!injectedWorker.includes('revision:23.0.0')||!injectedWorker.includes('build:experience-r1'))fail('V23 worker revision is stale');
...
const v23Css=fs.readFileSync(path.join(site,'app','pacefold-v23.css'),'utf8'),v23Runtime=fs.readFileSync(path.join(site,'app','pacefold-v23.js'),'utf8'),v23Bootstrap=fs.readFileSync(path.join(site,'app','pacefold-v23-bootstrap.js'),'utf8');
for(const token of ['@media(forced-colors:active)','@media(prefers-reduced-motion:reduce)','.pf23-seconds-dial','.pf22-settings-layout[data-hardened]','.pf23-schedule-strip','.pf23-context-schedule'])if(!v23Css.includes(token))fail(`V23 CSS contract missing: ${token}`);
for(const token of ["const RELEASE='23.0.0'",'__PACEFOLD_ACTIVE_RELEASE__','pacefold.spatial.note.draft.v1','window.__PACEFOLD_EXPERIENCE__','waterLastAt','gazeLastAt','bodyLastAt'])if(!v23Runtime.includes(token))fail(`V23 runtime contract missing: ${token}`);
for(const token of ['pf23-boot-hold','__PACEFOLD_BOOTSTRAP__','pacefoldPrefsV14'])if(!v23Bootstrap.includes(token))fail(`V23 bootstrap contract missing: ${token}`);
...
console.log(JSON.stringify({version,sha256:actualSha,parts:parts.length,hubPins:'verified',coreValidation:'passed',surface:'23.0.0 unified spatial workday',trustedTypes:'core and injected surfaces',offlineAssets:'verified',landingPage:'23.0.0',activeRequests:{styles:13,scripts:14},oneNoteDelivery:'retired',debugAuditBuild:'enabled'},null,2));
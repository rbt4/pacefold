...
assert(read('pacefold-experience.txt').trim()==='27.0.0 polish-r1','Build identity is wrong');
...
assert(mainSource.includes('installCueStore(ctx)')&&mainSource.includes('installEdges(ctx)')&&mainSource.includes('installRelease(ctx)'),'V27 runtime modules are not fully wired');
assert(cueSource.includes('notify-${iconName}-128.png')&&cueSource.includes('badge-96.png'),'Raster notification URLs are missing');
assert(cueSource.includes("document.title=ctx.currentCues.length?`Clock · ${ctx.currentCues.length}`:'Clock'"),'Ambient title is not discreet');
assert(!cueSource.includes('Waiting on the clock'),'Duplicate Clock cue header returned');
assert(worker.includes("const VERSION='27.0.0'")&&worker.includes('polish-r1'),'Service-worker cache identity is stale');
...
assert(stateSource.includes("RELEASE='27.0.0'")&&stateSource.includes("cueState:'pacefold.cues.v1'")&&stateSource.includes("JSON.stringify({v:1,items:ctx.notes"),'V27 identity or storage continuity is incomplete');
...
console.log(JSON.stringify({release:'27.0.0',revision:'polish-r1',runtimes:1,stylesheets:1,discreetChrome:'checked',backgroundPrivacy:'checked',dayComparison:'checked',weatherContext:'checked',movingNowMarker:'checked',storageContinuity:'checked',prayerSchedule:'passed'},null,2));

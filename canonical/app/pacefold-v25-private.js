...
function install(){
  ...
  for(const event of ['pacefold:prefs','pacefold:storage-changed','pacefold:experience-ready'])window.addEventListener(event,queue);window.addEventListener('pacefold:quiet',()=>{quietPass();queue()});window.addEventListener('pacefold:spatial-mode',()=>{queue();requestAnimationFrame(queue)});window.addEventListener('pacefold:spatial-ready',()=>{bindSpatialNavigation();queue();requestAnimationFrame(queue)});

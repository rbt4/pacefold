(() => {
  ...
  function renderOnboarding(){
    ...
    setHTML($('onboardingProgress'),[0,1,2].map((_,i)=>`<i class="${i<=onboardingStep?'active':''}"></i>`).join(''));
  ...
  function updateAppBadge(attention,states){
    if(window.__PACEFOLD_SCHEDULER__?.updateBadge?.(attention,states))return;
  ...
  function waterState(now,h){
    ...
    const complete=sips>=plan.total,paused=!!prefs.awayStart||!!prefs.lunchStart||!!prefs.prayerBreakStart,last=Math.max(Number(prefs.waterLastAt)||0,workdayStartAt(now)),since=Math.max(0,now.getTime()-last),remaining=Math.max(0,plan.cadence*60000-since),due=(prefs.workReminders&&within&&!paused&&!complete&&!cueSkipped('water',now)&&since>=plan.cadence*60000&&Date.now()>=prefs.waterGraceUntil)||testRoutine==='water';
  ...
  function gazeState(now,h){
    ...
    const last=Math.max(Number(prefs.gazeLastAt)||0,latestRecoveryAt(),base.getTime()),elapsed=Math.max(0,now.getTime()-last),due=prefs.gazeEnabled&&prefs.workReminders&&within&&currentDayType(now)!=='field'&&!cueSkipped('eyes',now)&&!paused&&Date.now()>=prefs.gazeSnoozedUntil&&elapsed>=prefs.gazeCadence*60000;
  ...
  function attentionFor(prayer,water,noodle,away,lunch,gaze,body){
    ...
    if(noodle.ready&&!field)return{signal:'due',source:'noodle'};
    ...
    if(body&&body.due&&!field)return{signal:'due',source:'body'};
    if(water.due)return{signal:'due',source:'water'};
    if(gaze&&gaze.due&&!field)return{signal:'due',source:'eyes'};
    ...
    if(away.active&&!field)return{signal:away.long?'due':'active',source:'away'};
    if(body&&body.active&&!field)return{signal:'active',source:'body'};
  ...
  window.__PACEFOLD_DELIVERY__=deliverSystemNotification;
  async function showSystemNotification(key,text,source='prayer',test=false,specOnly=false){if(!test&&window.__PACEFOLD_SCHEDULER__?.request)return window.__PACEFOLD_SCHEDULER__.request(key,text,source,test,specOnly);return deliverSystemNotification(key,text,source,test,specOnly);}
  ...
  function openFold(mode='capture'){
    foldMode=['capture','care','sound'].includes(mode)?mode:'capture';const drawer=$('foldDrawer');drawer.hidden=false;document.body.classList.add('fold-open');togglePanel(false);renderFold();
  ...
  function renderCore(){
    ...
    const minuteText=String(m).padStart(2,'0');if(!window.__PACEFOLD_VIEW__?.setMinute?.($('minute'),minuteText))$('minute').textContent=minuteText;$('seconds').textContent=String(s).padStart(2,'0');window.__PACEFOLD_VIEW__?.setSecondProgress?.(s);$('date').textContent=now.toLocaleDateString(undefined,{weekday:'long',month:'long',day:'numeric'});
    ...
    document.body.dataset.signal=attention.signal;document.body.dataset.source=attention.source;document.body.dataset.prayerSignal=prayer.signal;document.body.dataset.dayType=currentDayType(now);document.body.dataset.quiet=String(Boolean(prefs.quietMode));applyTheme(now,h);applyComfort(now,h);document.body.dataset.clarity=prefs.clarity;document.body.dataset.edge=prefs.edgeCue?'on':'off';document.body.dataset.seconds=prefs.showSeconds?'on':'off';if(!prefs.privacy)document.body.classList.add('reveal');
    ...
    window.__PACEFOLD_VIEW__?.applyStatus?.();
  ...
  function buildPanel(h,prayer,water,noodle,away,lunch){
    ...
    setHTML($('panel'),`
      ...
      <div class="section-title">Rhythm profile</div>${choices('Profile','profileSet',Object.entries(PROFILE_PRESETS).map(([id,x])=>[id,x.label]),prefs.profile)}<div class="profile-note">${profileMeta().note}</div>${!isMuslimProfile()?'<div class="actions"><button class="action wide" data-action="editMoments" type="button">Edit labels and times</button></div>':''}<div class="section-title">Today</div><div class="prayer-grid">${rows}</div>
      <div class="section-title">Display</div>
      ${choices('Theme','themeSet',[['auto','Auto'],['desk','Clear'],['paper','Soft'],['dark','Dark'],['moss','Moss'],['dusk','Dusk'],['custom','Custom']],prefs.theme)}
      ...
      <div class="section-title">Cue delivery</div>
      ...
      <div class="section-title">Focus sound</div>
      ...
      <div class="section-title">Day history</div><div class="timeline">${timelineHtml}</div>
      <div class="section-title">Day close</div>
  ...
  $('panel').addEventListener('click',async e=>{
    ...
    else if(a==='openOneNote'){openFold('capture');return;}

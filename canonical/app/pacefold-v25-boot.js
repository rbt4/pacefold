...
const VERSION_LABEL='Pacefold 25.0.0 · private local recovery';
...
window.addEventListener('pacefold:recovery-ready',()=>{
  ...
  const syncVersion=()=>{for(const node of document.querySelectorAll('.pf22-version'))if(node.textContent!==VERSION_LABEL)node.textContent=VERSION_LABEL};
  const sync=()=>{cancelAnimationFrame(frame);frame=requestAnimationFrame(()=>{window.__PACEFOLD_RECOVERY__?.refresh?.();syncVersion()})};
  ...
  for(const node of document.querySelectorAll('.pf22-version'))versionObserver.observe(node,{childList:true,characterData:true,subtree:true});
  syncVersion();
},{once:true});
})();
